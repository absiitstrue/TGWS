...
int tgws_draw_vertical_gradient(int r1, int g1, int b1,
    ...
    for (; cw< fgw; cw++){
        ...
        int r= round((float)r1+shr*ch);
        int g= round((float)g1+shg*ch);
        int b= round((float)b1+shb*ch);

        int rt= round((float)r1+shr*(ch+1));
        int gt= round((float)g1+shg*(ch+1));
        int bt= round((float)b1+shb*(ch+1));
        int ox = x + ch;
        int oy = y + cw;
